...
OUTPUT_DIR = './Users/lucaslankry/Desktop'
MODEL_FILE = 'saved_model'
VEC_NORM_FILE = 'vec_norm'
...
def train_model(save_to=OUTPUT_DIR):
    ...
    model = A2C(TraderActorCriticPolicy, env, verbose=1,
                          tensorboard_log='./tb_log', policy_kwargs=policy_kwargs)
    print(model.policy)
...
def load_model():
    price_feed = ARIMAPriceFeed(n_samples=episode_len, **time_series_config)
    env = DummyVecEnv([lambda: BrokerEnv(price_feed, **broker_config)])
